...
- **Vercel deploys `pkgs/app` only** (never `pkgs/cli`), driven by the root `vercel.json` (`bun run build:app`, output `pkgs/app/dist`) — see the README "Deploy `pkgs/app` to Vercel" section. Root Directory must stay at the repo root so `pkgs/contract`/`pkgs/shared` build first. As of this writing there is no production proof-server rewrite configured (the team chose to ship the UI first): `commit`/`reveal` will fail on the deployed site until someone adds a `rewrites` entry in `vercel.json` pointing `/proof-server/*` at a publicly reachable HTTPS proof server.
- **`pkgs/app/vite.config.ts`'s Node polyfill setup is load-bearing — don't casually add back `vm`/`stream` to the blanket `stdLibBrowser` alias spread, and don't remove `cjsInteropBuildShimPlugin`'s entries.** Several of node-stdlib-browser's legacy CJS polyfills (`util`, `assert`, crypto-browserify's whole tree pulled in via `crypto` — genuinely needed to encrypt the private-state LevelDB store, see `@midnight-ntwrk/midnight-js-level-private-state-provider` — plus `semver`, `path-browserify`, and even `@midnight-ntwrk/midnight-js-utils`'s own CJS build) get only *partially* transformed by `@rollup/plugin-commonjs` in production: some internal helper file is wrapped in an IIFE correctly, but the entry file's top-level `exports.foo = ...`/`module.exports = ...` is left as a bare, unbound reference, throwing `Uncaught ReferenceError: exports is not defined` in the browser the moment the page loads (`vite build`/`tsc` never catch this — it only shows up at runtime, since ESM has no implicit `exports`/`module` the way Node's CJS loader does). This was never caught before because nobody had loaded a real production build in an actual browser until the first Vercel deploy. `cjsInteropBuildShimPlugin` (in `vite.config.ts`) works around it by bundling each affected package with esbuild directly (esbuild's CJS→ESM interop handles the shape Rollup's doesn't) instead of letting Rollup's plugin-commonjs touch it; `vm`/`stream` are instead just dropped from the alias list entirely since nothing in this app actually imports them (they were dead weight only dragged in by the blanket alias). If you add a new node-stdlib-browser alias and the production bundle throws this error again, use the same pattern: bundle the offending package/polyfill through `cjsInteropBuildShimPlugin` rather than fighting `commonjsOptions` further (`transformMixedEsModules`/`strictRequires`/`include` were all tried first and none of them fixed it).
